### Script network diagram: an illustration to get an idea about miRNA-circRNA interactions
...
miRNA_sponging_coefficients <- read_csv("./miRNA sponging coefficient/Results/miRNA sponging coefficient.csv")
circRNAs_effective_sites <- read_csv("./miRNA sponging coefficient/Results/circRNAs effective sites.csv")
...
# Join the tables with the miRNAs sponging coefficients and circRNA effective sites
df <- left_join(miRNA_circRNA_Overlaps, miRNA_sponging_coefficients%>% select(-quartile))
df <- left_join(df, circRNAs_effective_sites %>% select(-quartile))

# To produce a network diagram we need to build edge list of interactions
    # We'll illustrate filtering for circRNAs with higher 'sponging' capacity
df <- subset(df, Total_Sites_Per_circ > 36000 & Ratio_TargetSites > 0.07)
df <- df[,c(16, 14)]
...
# Assign colors
V(network)$color <- adjustcolor("#44015495", alpha.f = .8)
V(network)[1:26]$color <- "#fde72595"
