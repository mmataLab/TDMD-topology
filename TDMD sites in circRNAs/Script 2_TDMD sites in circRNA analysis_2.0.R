...
packages = c("tidyverse", "ggpubr", "viridis", "emmeans", "rstatix")
...
circRNA_expression_FB <- read.csv("miRNA sponging coefficient/circRNA_expression_FB_GSE73325.csv", sep = ";", header = TRUE)
  # Sites obtained with scanMiR using Script 1
...
# Rename columns w/dplyr: 
circRNA_expression_H9 <- dplyr::rename(circRNA_expression_H9, "Chromosome" = "ï..Chromosome")
circRNA_expression_FB <- dplyr::rename(circRNA_expression_FB, "Chromosome" = "ï..Chromosome")
...
write_csv(sites_per_miR, "./TDMD sites in circRNAs/Results/TDMD sites on circs per miR_2.0.csv")
