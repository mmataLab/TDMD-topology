...
# Calculate a relative amount of target sites based on how much the circRNA is expressed (average between before and after diff.) and how many sites it has for a given miRNA
miRNA_circRNA_Overlaps$Relative_TargetSites <- ((miRNA_circRNA_Overlaps$H9_JReads + miRNA_circRNA_Overlaps$FB_JReads)/2) * miRNA_circRNA_Overlaps$Sites_per_circ
...
most_sponged_miRNAs$Sponging_Coefficient <- most_sponged_miRNAs$Total_TargetSites / ((most_sponged_miRNAs$H9_RPM + most_sponged_miRNAs$FB_RPM)/2)
...
ggl <- ggplot(data = most_sponged_miRNAs, aes(x = Total_TargetSites, y = log2FoldChange_miR)) +
  ...
  xlab("miRNA-specific 'Effective'\nsites on all circRNAs") + ylab("miRNA Fold change (log2)") + 
...
gspo <- ggplot(data = most_sponged_miRNAs, aes(x = log10(Sponging_Coefficient), y = log2FoldChange_miR)) +
  geom_point(alpha = 0.8, aes(color = quartile, size = (H9_RPM + FB_RPM)/2)) +
  geom_hline(yintercept =  0, color = "grey", alpha = 0.8) +
  stat_cor(method = "pearson") +
  scale_color_viridis(option = "D", direction = -1, discrete = TRUE, guide = "none") +
  scale_size(name = "miRNA expression\n           (RPM)", limits = c(1,150000), breaks = c(1000, 150000)) +
  theme_pubr(base_family = "Myriad Pro", base_size = 18) + 
  geom_text_repel(data = subset(most_sponged_miRNAs, miRNAname == "hsa-miR-7-5p"),
                  ...
                  nudge_x = 0.3, nudge_y = 8, arrow = arrow(angle = 10, length = unit(0.1, "inches"))) +
  theme(legend.text = element_text(size = 9),
        legend.title = element_text(size = 14),
        legend.key.width = unit(0.02, 'cm')) +
  xlab("'Sponging' suffered\ncoefficient (log10)") + ylab("miRNA Fold change (log2)") +
  ylim(-20,20)
gspo
...
write_csv(most_sponged_miRNAs, "./miRNA sponging coefficient/Results/miRNA sponging coefficient_2.0.csv")
write_csv(miRNA_circRNA_Overlaps, "./miRNA sponging coefficient/Results/miR_Cir_interactions_fulltable_2.0.csv")
