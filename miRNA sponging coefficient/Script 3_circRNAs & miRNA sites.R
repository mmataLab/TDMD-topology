...
most_sponging_circRNAs <- miRNA_circRNA_Overlaps %>% 
  ...
  slice(1) %>%
  ungroup() %>%
  group_by(Chromosome, Start, End, Strand, H9_JReads, geneName, H9_RPKM, log2FoldChange_mRNA, log2FoldChange_circ) %>% 
  summarise(Total_Sites_Per_circ = sum(Sites_per_circ))

# Consider the effective amount of sites by multiplying the number of sites by the expression of the circRNA pre-differentiation
...
miRNA_circRNA_Overlaps <- left_join(miRNA_circRNA_Overlaps, miRNA_sponging_coefficients)
...
write_csv(most_sponging_circRNAs, "./miRNA sponging coefficient/Results/circRNAs effective sites.csv")
